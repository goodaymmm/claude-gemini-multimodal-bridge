...
export class PromptOptimizer {
  ...
  /**
   * Optimize prompt for WebSearch by shortening
   */
  ...
  private applySearchTemplate(prompt: string): { matched: boolean; optimized: string } {
    ...
    for (const [templateName, template] of Object.entries(this.SEARCH_TEMPLATES)) {
      ...
      if (matchedKeywords.length >= 2) {
        ...
        // Append additional important keywords
  ...
  private extractKeywords(prompt: string): string {
    ...
    // Pattern matching for important phrases
  ...
  private removeRedundancy(prompt: string): string {
    return prompt
      ...
      // Normalize duplicate characters
      .replace(/[。、]{2,}/g, '、')
      .replace(/\s{2,}/g, ' ')
      .trim();
  }
  ...
  private structuredShortening(prompt: string, maxTokens: number): string {
    ...
    // Select sentences within token limit
  ...
  private estimateTokens(text: string): number {
    ...
    // Hiragana/Katakana: 1 character ≈ 1 token
